...
class FitJmaxVcmaxKnownRdark(FitMe):
    """ Fit the model parameters Jmax, Vcmax to the measured A-Ci and Rdark
        data.
    ...
    def __init__(self, model=None, ofname=None, results_dir=None,
        ...
        """
        FitMe.__init__(self, model, ofname, results_dir, data_dir, plot_dir)
        self.header = ["Jmax", "JSE", "Vcmax", "VSE", "Tav", "Var", "R2", \
                     "SSQ", "MSE", "DOF", "n", "Species", "Season", \
                     "Leaf", "Curve", "Filename", "id"]
        # Rday = 0.6 x Rdark (Kirschbaum & Farquhar, 1984)
        self.Rdark2Rday = 0.6
    ...
    def main(self, print_to_screen, infname_tag="*.csv"):
        ...
        for fname in glob.glob(os.path.join(self.data_dir, infname_tag)):
            ...
            # Clean up the Rdark -> Rd data
            #
            # Need to make these numbers positive if they are negative, we
            # are assuming
            data["Rdark"] = np.where(data["Rdark"] < 0.0,
                                     (data["Rdark"] * -1.0) * self.Rdark2Rday,
                                      data["Rdark"])
            # Fix name of Rd
            data.dtype.names = 'Curve', 'Tleaf', 'Ci', 'Photo', 'Species', \
                               'Season', 'Leaf', 'fitgroup', 'Rd'

            data["Tleaf"] += self.deg2kelvin
            for curve_num in np.unique(data["Curve"]):
                curve_data = data[np.where(data["Curve"]==curve_num)]
                (vcmax_guess, jmax_guess) = self.pick_starting_point(curve_data)
                params = self.setup_model_params(jmax_guess=jmax_guess,
                                                 vcmax_guess=vcmax_guess,
                                                 rd_guess=None)
    ...
    def pick_starting_point(self, data, grid_size=100):
        ...
        p1, p2 = np.ix_(Jmax, Vcmax)
        if hasattr(data, "Par"):
            (An, Anc, Anj) = self.call_model(Ci=data["Ci"][:,None,None],
                                             Tleaf=data["Tleaf"][:,None,None],
                                             Par=data["Par"][:,None,None],
                                             Jmax=p1, Vcmax=p2,
                                             Rd=data["Rd"][:,None,None])
        else:
            (An, Anc, Anj) = self.call_model(Ci=data["Ci"][:,None,None],
                                             Tleaf=data["Tleaf"][:,None,None],
                                             Jmax=p1, Vcmax=p2,
        ...
        rmse = np.sqrt(((data["Photo"][:,None,None]- An)**2).mean(0))
        ndx = np.where(rmse.min()== rmse)
        (jidx, vidx) = ndx

        return Vcmax[vidx].squeeze(), Jmax[jidx].squeeze()
